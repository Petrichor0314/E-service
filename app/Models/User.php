   ...
   public static function getStudentsClass($class_id)
{
    ...
    static public function getStudentExport()
    {
        $return = self::select('users.id','users.name','users.last_name','users.gender','users.CIN','users.CNE','class.name as class_name','users.date_of_birth','users.admission_date','users.mobile_number','users.email')
                        ->join('class','class.id','=','users.class_id','left')
                        ->where('users.user_type','=',3)
